...
ADMIN_EMAIL='hello@elmima.marketing'
...
install_mailhog() {
	echo "Changing into ${VH_DOC_ROOT}"
	pushd "${VH_DOC_ROOT}"
	echo "Setting up the Wordpress Instance"
	wp --allow-root core install --url=${DOMAIN} --title=${DOMAIN} --admin_user=${ADMIN_USER} --admin_password=${ADMIN_PASSWORD} --admin_email=${ADMIN_EMAIL}
	echo "Wordpress is installed with admin use ${ADMIN_USER} and password ${ADMIN_PASSWORD}"
	echo "Installing Mailhog Plugin"
	wp --allow-root plugin install wp-mailhog-smtp
	cat << EOD > mailhog-const.txt
...
EOD
	sed -i "/\/\* That's all, stop editing! Happy publishing. \*\//r  mailhog-const.txt" ./wp-config.php	
	wp --allow-root plugin activate wp-mailhog-smtp
	rm ./mailhog-const.txt
	chown -R "${WWW_UID}:${WWW_GID}" "${VH_DOC_ROOT}"
	popd
}

clean_plugins() {
	pushd "${VH_DOC_ROOT}/wp-content/plugins"
	rm -rf *
	echo "BEARNY WAS HERE" >index.php
	popd
}
...
while [ ! -z "${1}" ]; do
	case ${1} in
		...
		-vhname | --vhname) shift
			VHNAME="${1}"
			;;
		-admin_user | --admin_user) shift
			ADMIN_USER="${1}"
			;;
		-admin_pass | --admin_pass) shift
			ADMIN_PASSWORD="${1}"	
			;;
		-admin_email | --admin_email) shift
			ADMIN_EMAIL="${1}"
			;;       
		*) 
			help_message
